...
public class IntrospectionClient(Func<HttpMessageInvoker> client, IntrospectionClientOptions options)
{
    private readonly Func<HttpMessageInvoker> _client = client ?? throw new ArgumentNullException(nameof(client));
    private readonly IntrospectionClientOptions _options = options ?? throw new ArgumentNullException(nameof(options));
